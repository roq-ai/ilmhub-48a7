...
import { getInformation } from 'apiSdk/information';
...
import { getWhiteboards } from 'apiSdk/whiteboards';
import { WhiteboardInterface } from 'interfaces/whiteboard';

import { InformationListPage as ShowCaseList } from 'pages/information';

function DashboardPage() {
  const { session } = useSession();

  const informationStat = useSWR<PaginatedInterface<InformationInterface>>(
    () => `/information`,
    () => getInformation({ limit: 1, offset: 0 }),
  );
  ...
  const whiteboardStat = useSWR<PaginatedInterface<WhiteboardInterface>>(
    () => `/whiteboards`,
    () => getWhiteboards({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Information',
      result: informationStat?.data?.totalCount,
      isLoading: informationStat.isLoading,
    ...
    {
      title: 'Whiteboards',
      result: whiteboardStat?.data?.totalCount,
      isLoading: whiteboardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
