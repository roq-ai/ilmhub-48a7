...
import { getWorkspaces, deleteWorkspaceById } from 'apiSdk/workspaces';
import { WorkspaceInterface } from 'interfaces/workspace';

type ColumnType = ColumnDef<WorkspaceInterface, unknown>;
...
export function WorkspaceListPage(props: WorkspaceListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getWorkspaces({
        relations: ['user', 'invitation.count', 'section.count', 'whiteboard.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<WorkspaceInterface>>(
    () => `/workspaces?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: WorkspaceInterface) => {
    if (hasAccess('workspace', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/workspaces/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('section', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Section',
          accessorKey: 'section',
          cell: ({ row: { original: record } }: any) => record?._count?.section,
        }
      : null,
    hasAccess('whiteboard', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'whiteboard',
          header: 'Whiteboard',
          accessorKey: 'whiteboard',
          cell: ({ row: { original: record } }: any) => record?._count?.whiteboard,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/workspaces/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('workspace', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/workspaces/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('workspace', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
