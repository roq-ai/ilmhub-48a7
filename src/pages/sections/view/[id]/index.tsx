...
import { getSectionById } from 'apiSdk/sections';
import { SectionInterface } from 'interfaces/section';
import { InformationListPage } from 'pages/information';

function SectionViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<SectionInterface>(
    () => (id ? `/sections/${id}` : null),
    () =>
      getSectionById(id, {
        relations: ['workspace'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('section', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/sections/edit/${id}`} passHref legacyBehavior>
                    <Button
