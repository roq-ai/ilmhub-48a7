...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<WorkspaceInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/workspaces` : null),
    () => getWorkspaces({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Workspaces',
      path: isTenantUser ? `/workspaces/view/${data?.[0]?.id}` : '/workspaces',
      entity: 'workspace',
    ...
    { name: 'Sections', path: '/sections', entity: 'section', service: AccessServiceEnum.PROJECT, icon: FiBookOpen },
    {
      name: 'Whiteboards',
      path: '/whiteboards',
      entity: 'whiteboard',
